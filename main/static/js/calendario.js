document.addEventListener("DOMContentLoaded", function () {
  ...
  function initializeCalendar() {
    calendarInstance = new FullCalendar.Calendar(calendarEl, {
      ...
      dateClick: function (info) {
        const eventOnDate = calendarInstance
          .getEvents()
          .find((e) => e.startStr.split("T")[0] === info.dateStr);
        if (eventOnDate) {
  ...
  async function fetchProgrammedRoutines(
    fetchInfo,
    successCallback,
    failureCallback
  ) {
    try {
      const response = await fetch(
        "http://localhost/Tp_Final_Interfaces/backend/api/rutinas/obtener_rutinas_programadas.php",
        {
          credentials: "include",
        }
      );
  ...
  async function showRoutineForDate(routineId, dayIndex) {
    ...
    try {
      const response = await fetch(
        `https://myproyectTPinterfaces.infy.uk/backend/api/rutinas/obtener_rutinas.php?id=${routineId}`,
        {
          credentials: "include",
        }
      );
  ...
  function displayNoRoutine(dateStr, message = "No hay rutina programada") {
    ...
    routineSection.innerHTML = `
            ...
            <div class="no-rutina">
                ...
                <p>${message}</p>
                <button class="btn-crear-rutina" onclick="window.location.href='/crear_rutina'">
  ...
  function updateRoutineViewForToday() {
    ...
    const eventOnDate = calendarInstance
      .getEvents()
      .find((e) => e.startStr.split("T")[0] === todayStr);
    if (eventOnDate) {
      const routineId = eventOnDate.id;
      const dayIndex = eventOnDate.extendedProps.dayIndex;
      showRoutineForDate(routineId, dayIndex);
  ...
  function setupCustomNavigation() {
    ...
    function updateTitle() {
      ...
      monthTitle.innerHTML = date.toLocaleDateString("es", {
        month: "long",
        year: "numeric",
      });
    }
  ...
  function inicializarBotonBorrarTodas() {
    ...
    btn.onclick = async function () {
      const confirmar = confirm(
        "¿Seguro que quieres borrar TODAS las rutinas programadas? Esta acción no se puede deshacer."
      );
      if (!confirmar) return;

      try {
        const response = await fetch(
          "http://localhost/TP_FINAL_INTERFACES/backend/api/rutinas/borrar_todas_rutinas.php",
          {
            method: "POST",
            credentials: "include",
        ...
        if (result.success) {
          ...
          alert(
            "❌ Error: " +
              (result.mensaje || "No se pudieron borrar las rutinas.")
          );
        }
      } catch (e) {
